...
public class BoardController implements BoardUI {
    ...
    private final Map<Piece, WinnerNotificationStrategy> winnerNotificationStrategies;

    public BoardController() {
        winnerNotificationStrategies = new HashMap<>();
        winnerNotificationStrategies.put(Piece.BLUE, new HumanWinStrategy());
        winnerNotificationStrategies.put(Piece.GREEN, new AIWinStrategy());
        winnerNotificationStrategies.put(Piece.EMPTY, new TieStrategy());
    }
    ...
    public void initialize() {
        initializeGame();
        grpCols.getChildren().stream()
               .map(n -> (VBox) n)
               .forEach(vbox -> vbox.setOnMouseClicked(mouseEvent -> colOnClick(vbox)));
    }
    ...
    public void update(int col, boolean isHuman) {
        ...
        animateCircleDrop(circle);
        updateGameStatus(isHuman, vCol);
    }

    private void animateCircleDrop(Circle circle) {
    ...
    private void updateGameStatus(boolean isHuman, VBox vCol) {
    ...
    private void handleHumanTurn() {
        ...
        grpCols.getChildren().stream()
               .map(n -> (VBox) n)
               .forEach(vbox -> vbox.getStyleClass().remove("col-human"));
        
        KeyFrame delayFrame = new KeyFrame(Duration.millis(300), 
            actionEvent -> {
    ...
    public void notifyWinner(Winner winner) {
        ...
        WinnerNotificationStrategy strategy = winnerNotificationStrategies.get(winner.getWinningPiece());
        lblStatus.setText(strategy.getWinnerMessage(playerName));

        if (winner.getWinningPiece() != Piece.EMPTY) {
            showWinningLine(winner);
        }
    ...
    private void showWinningLine(Winner winner) {
        VBox vCol = (VBox) grpCols.lookup("#col" + winner.getCol1());
        Rectangle rect = new Rectangle(
            (winner.getCol2() - winner.getCol1() + 1) * vCol.getWidth(),
